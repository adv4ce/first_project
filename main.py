...
class Upload:
    def __init__(self, token, count_photos=5, folder_name='Homework photos',
    ...
    def create_folder(self):
        try:
            requests.put(f'{self.upload_url}?path=disk%3A%2F{self.folder_name}', headers=self.headers)
        except requests.RequestException as e:
            print(f"Ошибка создания папки: {e}")
            return requests.put(f'{self.upload_url}?path=disk%3A%2F{self.folder_name}', headers=self.headers)

    def upload_photos(self, replace=False):
        for upload_photo in tqdm(os.listdir('user_photos')[:self.count_photos], desc='Uploading photos', unit=' photo'):
            res = requests.get(
...
if __name__ == "__main__":
    ...
    type_photos = int(input('Выберите цифру в зависимости от типа фотографий, которые вы хотите загрузить:\n'
    ...
    if select_type == ValueError:
        while select_type == ValueError:
            select_type = int(input('Ошибка. Выберите правильное число из списка:\n'
                                    '1 - profile\n'
                                    '2 - wall\n'
                                    '3 - saved\n'))
    ...
    count_photos = int(input(f'Введите количество фото для загрузки (Максимум {len(os.listdir("user_photos"))}): '))
    ...
    try:
        ...
        print(f'Фотографии в количестве {count_photos} шт успешно загружены на ваш Яндекс Диск')
